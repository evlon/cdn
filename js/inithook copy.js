(function() {
    ...
    function reExamPageIniter(lastExamId) {
        ...
        win.submitAnswer = function (){
            ...
            var qformData = {
                ...
                examRecordId:formData.examRecordId,
                paperInstanceId:qmap.allData.examRecord.paperInstanceId,
                fullAnswerRecords:formData.fullAnswerRecords,
            };
            ...
            win.$.post('/api/v1/exam/exam-record/front/submitPaper',formData,function(json){
                if(json.status == 1){
                    win.alert('提交成功!')
                    win.$.post('/api/v1/exam/exam-record/my-score',qformData,function(qjson){
                        win.alert('您的分数:' + (qjson.score/ 100.0).toFixed(2));
                    });
    ...
    $('a[data-resource-id]').each(function(i, el) {
        ...
        if (text.indexOf('重新考试') != -1 || text.indexOf('开始考试') != -1) {
            var $btn = $('<a href="javascript:void(0)" title="原题开卷考试,请先开始考试，然后生出来一半时间提交后开始">开卷考试</div>');
            $el.append($btn);
